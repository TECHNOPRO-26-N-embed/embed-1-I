...
[電源ON / 初期化]
  ↓（音楽無限再生開始）
[待機中] ──（ボタン、ポテンションメーター検知中）──→ [計測中] ──（ボタンが押される、ポテンションメーターの値が下限/上限に達する）──→ [音楽を停止/再生、音量を下げる/上げる、音楽を一曲先に/一曲前に]

？　全ての状態から抜け出せるルートがあるか確認してくださいと書いてあるけど、音楽再生の場合は抜け出せないルートがあるからこそいいのでは。電源ボタンがいるということなのだろうか。
